...
#ifndef OPENFST_EXTENSIONS_MPDT_MPDT_H_
#define OPENFST_EXTENSIONS_MPDT_MPDT_H_
...
class MPdtStack {
 ...
 private:
  ...
  absl::flat_hash_map<Label, Label> paren_levels_;
  std::vector<std::pair<Label, Label>> parens_;   // As in pdt.h.
  absl::flat_hash_map<Label, size_t> paren_map_;  // As in pdt.h.
  // Maps between internal paren_id and external paren_id.
  absl::flat_hash_map<KeyPair<Level>, size_t, KeyPairHasher<Level>>
      paren_id_map_;
  // Maps between internal stack ids and external stack id.
  ConfigToStackId config_to_stack_id_map_;
  absl::flat_hash_map<StackId, Config> stack_id_to_config_map_;
...
#endif  // OPENFST_EXTENSIONS_MPDT_MPDT_H_

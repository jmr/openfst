...
ABSL_FLAG(std::string, fst_weight_separator, ",",
          "Character separator between printed composite weights; "
          "must be a single character");

ABSL_FLAG(std::string, fst_weight_parentheses, "",
...
CompositeWeightIO::CompositeWeightIO()
    : CompositeWeightIO(
          absl::GetFlag(FLAGS_fst_weight_separator).empty()
              ? 0
              : absl::GetFlag(FLAGS_fst_weight_separator).front(),
          {absl::GetFlag(FLAGS_fst_weight_parentheses).empty()
               ? 0
               : absl::GetFlag(FLAGS_fst_weight_parentheses)[0],
           absl::GetFlag(FLAGS_fst_weight_parentheses).size() < 2
               ? 0
               : absl::GetFlag(FLAGS_fst_weight_parentheses)[1]}) {
  if (absl::GetFlag(FLAGS_fst_weight_separator).size() != 1) {
    FSTERROR() << "CompositeWeight: "
               << "FLAGS_fst_weight_separator.size() is not equal to 1";
    error_ = true;
  }
  if (!absl::GetFlag(FLAGS_fst_weight_parentheses).empty() &&
      absl::GetFlag(FLAGS_fst_weight_parentheses).size() != 2) {
    FSTERROR() << "CompositeWeight: "

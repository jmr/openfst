...
#ifndef OPENFST_LIB_SYNCHRONIZE_H_
#define OPENFST_LIB_SYNCHRONIZE_H_
...
// TODO: Refactor to guarantee thread-safety.
...
class SynchronizeFstImpl : public CacheImpl<Arc> {
 ...
 private:
  ...
  using StringSet = absl::node_hash_set<String, StringKey>;
...
void Synchronize(const Fst<Arc> &ifst, MutableFst<Arc> *ofst) {
  ...
  const SynchronizeFstOptions opts(absl::GetFlag(FLAGS_fst_default_cache_gc),
                                   0);
...
#endif  // OPENFST_LIB_SYNCHRONIZE_H_

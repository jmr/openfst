...
#ifndef OPENFST_SCRIPT_FST_CLASS_H_
#define OPENFST_SCRIPT_FST_CLASS_H_
...
class FstClass : public FstClassBase {
 public:
  ...
  static absl_nullable std::unique_ptr<FstClass> Read(
...
class MutableFstClass : public FstClass {
 public:
  ...
  static absl_nullable std::unique_ptr<MutableFstClass> Read(
  ...
  explicit MutableFstClass(std::unique_ptr<MutableFst<Arc>> fst)
      ...
      // arcane reason.
      : FstClass(
            absl::implicit_cast<std::unique_ptr<Fst<Arc>>>(std::move(fst))) {}
...
class VectorFstClass : public MutableFstClass {
 public:
  ...
  explicit VectorFstClass(absl::string_view arc_type);

  static absl_nullable std::unique_ptr<VectorFstClass> Read(
      const std::string &source);
  ...
  explicit VectorFstClass(std::unique_ptr<VectorFst<Arc>> fst)
      // NB: The natural cast-less way to do this doesn't compile for some
      // arcane reason.
      : MutableFstClass(absl::implicit_cast<std::unique_ptr<MutableFst<Arc>>>(
            std::move(fst))) {}
...
class FstClassIORegister
 ...
 public:
  Reader GetReader(absl::string_view arc_type) const {
    return this->GetEntry(arc_type).reader;
  }

  Creator GetCreator(absl::string_view arc_type) const {
    return this->GetEntry(arc_type).creator;
  }

  Converter GetConverter(absl::string_view arc_type) const {
...
#endif  // OPENFST_SCRIPT_FST_CLASS_H_

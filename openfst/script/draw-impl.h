...
#ifndef OPENFST_SCRIPT_DRAW_IMPL_H_
#define OPENFST_SCRIPT_DRAW_IMPL_H_
...
class FstDrawer {
 public:
  ...
  FstDrawer(const Fst<Arc> &fst, const SymbolTable *isyms,
            ...
            absl::string_view title, float width, float height, bool portrait,
            bool vertical, float ranksep, float nodesep, int fontsize,
            int precision, absl::string_view float_format,
  ...
  void Draw(std::ostream &strm, absl::string_view dest) {
    SetStreamState(strm);
 ...
 private:
  ...
  static std::string Escape(absl::string_view str) {
    std::string ns;
    for (char c : str) {
      if (c == '\\' || c == '"') ns.push_back('\\');
      ns.push_back(c);
...
#endif  // OPENFST_SCRIPT_DRAW_IMPL_H_

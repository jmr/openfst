...
#ifndef OPENFST_SCRIPT_PRINT_H_
#define OPENFST_SCRIPT_PRINT_H_
...
#include "openfst/script/print-impl.h"
...
void PrintFst(const Fst<Arc> &fst, std::ostream &ostrm,
              absl::string_view dest = "", const SymbolTable *isyms = nullptr,
              const SymbolTable *osyms = nullptr,
              const SymbolTable *ssyms = nullptr) {
  const std::string sep = absl::GetFlag(FLAGS_fst_field_separator).substr(0, 1);

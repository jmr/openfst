...
class WeightTester {
 ...
 private:
  ...
  void TestSemiring(Weight w1, Weight w2, Weight w3) {
    ...
    CHECK(Plus(w1, w2).Member());  // Crash OK
    CHECK(Times(w1, w2).Member());  // Crash OK

    // Checks that the operations are associative.
    CHECK(  // Crash OK
        ApproxEqual(Plus(w1, Plus(w2, w3)), Plus(Plus(w1, w2), w3)));
    CHECK(  // Crash OK
        ApproxEqual(Times(w1, Times(w2, w3)), Times(Times(w1, w2), w3)));

    // Checks the identity elements.
    CHECK(Plus(w1, Weight::Zero()) == w1);  // Crash OK
    CHECK(Plus(Weight::Zero(), w1) == w1);  // Crash OK
    CHECK(Times(w1, Weight::One()) == w1);  // Crash OK
    CHECK(Times(Weight::One(), w1) == w1);  // Crash OK

    // Check the no weight element.
    CHECK(!Weight::NoWeight().Member());  // Crash OK
    CHECK(!Plus(w1, Weight::NoWeight()).Member());   // Crash OK
    CHECK(!Plus(Weight::NoWeight(), w1).Member());   // Crash OK
    CHECK(!Times(w1, Weight::NoWeight()).Member());  // Crash OK
    CHECK(!Times(Weight::NoWeight(), w1).Member());  // Crash OK

    // Checks that the operations commute.
    CHECK(ApproxEqual(Plus(w1, w2), Plus(w2, w1)));  // Crash OK

    if (Weight::Properties() & kCommutative)
      CHECK(ApproxEqual(Times(w1, w2), Times(w2, w1)));  // Crash OK

    // Checks Zero() is the annihilator.
    CHECK(Times(w1, Weight::Zero()) == Weight::Zero());  // Crash OK
    CHECK(Times(Weight::Zero(), w1) == Weight::Zero());  // Crash OK

    // Check Power(w, 0) is Weight::One()
    CHECK(Power(w1, 0) == Weight::One());  // Crash OK

    // Check Power(w, 1) is w
    CHECK(Power(w1, 1) == w1);  // Crash OK

    // Check Power(w, 2) is Times(w, w)
    CHECK(Power(w1, 2) == Times(w1, w1));  // Crash OK

    // Check Power(w, 3) is Times(w, Times(w, w))
    CHECK(Power(w1, 3) == Times(w1, Times(w1, w1)));  // Crash OK

    // Checks distributivity.
    if (Weight::Properties() & kLeftSemiring) {
      CHECK(ApproxEqual(Times(w1, Plus(w2, w3)),  // Crash OK
                        Plus(Times(w1, w2), Times(w1, w3))));
    }
    if (Weight::Properties() & kRightSemiring)
      CHECK(ApproxEqual(Times(Plus(w1, w2), w3),  // Crash OK
                        Plus(Times(w1, w3), Times(w2, w3))));

    if (Weight::Properties() & kIdempotent) CHECK(  // Crash OK
            Plus(w1, w1) == w1);

    if (Weight::Properties() & kPath)
      CHECK(Plus(w1, w2) == w1 || Plus(w1, w2) == w2);  // Crash OK

    // Ensure weights form a left or right semiring.
    CHECK(  // Crash OK
        Weight::Properties() & (kLeftSemiring | kRightSemiring));

    // Check when Times() is commutative that it is marked as a semiring.
    if (Weight::Properties() & kCommutative)
      CHECK(Weight::Properties() & kSemiring);  // Crash OK
  }
  ...
  void TestDivision(Weight w1, Weight w2) {
    ...
    if (Weight::Properties() & kLeftSemiring) {
      ...
      if (d.Member()) CHECK(ApproxEqual(p, Times(w1, d)));  // Crash OK
      CHECK(!Divide(w1, Weight::NoWeight(), DIVIDE_LEFT).Member());  // Crash OK
      CHECK(!Divide(Weight::NoWeight(), w1, DIVIDE_LEFT).Member());  // Crash OK
    }

    if (Weight::Properties() & kRightSemiring) {
      Weight d = Divide(p, w2, DIVIDE_RIGHT);
      if (d.Member()) CHECK(ApproxEqual(p, Times(d, w2)));  // Crash OK
      CHECK(!Divide(w1, Weight::NoWeight(), DIVIDE_RIGHT).Member());  // Crash OK
      CHECK(!Divide(Weight::NoWeight(), w1, DIVIDE_RIGHT).Member());  // Crash OK
    }

    if (Weight::Properties() & kCommutative) {
      Weight d1 = Divide(p, w1, DIVIDE_ANY);
      if (d1.Member()) CHECK(ApproxEqual(p, Times(d1, w1)));  // Crash OK
      Weight d2 = Divide(p, w2, DIVIDE_ANY);
      if (d2.Member()) CHECK(ApproxEqual(p, Times(w2, d2)));  // Crash OK
    }
  }
  ...
  void TestIO(Weight w) {
    ...
    // Tests textual I/O.
    {
      ...
      Weight v(Weight::One());
      is >> v;
      CHECK(ApproxEqual(w, v));  // Crash OK
    }
  }
